...
class SentryAccessibilityService : AccessibilityService() {
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        // 1. ALWAYS ALLOW OUR OWN APP ACTIVITIES (MainActivity, EventsActivity, etc.)
        if (pkgName == packageName) {
            SentryService.isAppInForeground = true
            return
        }

        // 2. PREVENT TURN OFF (Block Power Off / Restart Dialogs Instantly)
        ...
        // 3. ALLOW SYSTEM UI ROTATION / SYSTEM DIALOGS (Prevents Orientation Flip Loops)
        if (pkgName == "com.android.systemui" || 
            pkgName == "android" || 
            pkgName.startsWith("com.oplus.") ||
            pkgName.startsWith("com.coloros.") ||
            pkgName.contains("inputmethod") || 
            pkgName == "com.android.settings") {
            return
        }

        // 4. BLOCK ALL UNAUTHORIZED EXTERNAL APPS (Home, YouTube, Games, WhatsApp)
        bringAppToFront()
    }

    private fun bringAppToFront() {
        if (!SentryService.isAppInForeground) {
